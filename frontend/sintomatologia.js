...
function coletarDadosSintomas() {
  ...
  const rows = document
    .getElementById("sintomasTableBody")
...
function exibirDadosPaciente() {
  ...
  if (pacienteData && Object.keys(pacienteData).length > 0) {
    ...
    if (pacienteData.alergiasPaciente) {
      dadosPacienteStr += `\nAlergias: ${JSON.stringify(
        pacienteData.alergiasPaciente,
        null,
        2
      )}\n`;
    }
    ...
    if (pacienteData.antecedentesClinicos) {
      dadosPacienteStr += `Antecedentes Clínicos: ${JSON.stringify(
    ...
    if (pacienteData.antecedentesFamiliares) {
      dadosPacienteStr += `Antecedentes Familiares: ${JSON.stringify(
    ...
    if (pacienteData.examesLaboratoriais) {
      dadosPacienteStr += `\nExames Laboratoriais: ${JSON.stringify(
        pacienteData.examesLaboratoriais,
        null,
        2
      )}`;
    }
...
function loadFormData() {
  ...
  if (pacienteData) {
    ...
    document.getElementById("displayNomePaciente").textContent =
      prontuarioPaciente.nome || "";
    document.getElementById("displayDataNascimento").textContent =
      prontuarioPaciente.dataNascimento || "";
    document.getElementById("displayEndereco").textContent =
  ...
  if (pacienteData.sintomatologia) {
    pacienteData.sintomatologia.forEach((sintomaData) => {
      const checkbox = document.getElementById(
        sintomaData.sintoma.toLowerCase().replace(/\s+/g, "_")
      );
